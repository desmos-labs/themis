...
def get_urls_from_bio(user: str) -> [str]:
    ...
    :param user: Login name of the Twitch user.
    :return: List of URLs that are found inside the bio
    """
...
def main(args: str):
    """
    Gets the signature data from Twitch reading it from a biography.

    :param args Hex encoded JSON object containing the arguments to be used during the execution.
    In order to be valid, the encoded JSON object must contain a field named "username" that represents the
    Twitch username of the user to be verified.
    ...
    :raise Exception if anything is wrong during the process. This can happen if:
            1. The user bio does not contain any valid URL that link to a valid signature data object
    ...
    # Get the URLs to check from the user bio
    urls = get_urls_from_bio(call_data.username)
    if len(urls) == 0:
